      ...
      <div className="max-w-3xl mx-auto mt-24">
        ...
        {data.repository ? (
          ...
          <a
            ...
            className="flex w-fit items-center gap-2 border border-gray-500 px-3 py-2 rounded transition cursor-pointer text-white hover:bg-white hover:text-black"
          >
            <FaGithub className="transition-colors duration-200" />
            See Repository
          </a>
        ) : data.web ? (
          // Case 2: Website exists
          <a
            href={data.web}
            target="_blank"
            rel="noopener noreferrer"
        ...
        <div className="grid grid-cols-3 gap-4 mt-6">
            ...
            <div className="flex-1 bg-[#151515] p-6 rounded-2xl hidden lg:flex flex-wrap gap-4 items-center">
              <h3 className="w-full text-center text-lg font-bold mb-2">Stack</h3>
              {data.stack.map((item, i) => (
                <ServicesItem key={i} name={item} />
              ))}
            </div>
          </div>
        ...
        <div className="mt-6 flex flex-col items-center justify-center p-4 rounded-lg w-full max-w-xl mx-auto">
          ...
          <h2 className="text-3xl font-semibold mt-8">See more projects</h2>
        </div>
        <div
        ...
        >
          {relatedProjects.map((work) => (
            <div
              key={work?.id}
              className="relative rounded-xl overflow-hidden shadow-lg group bg-[#151515] transition-all duration-500 ease-in-out hover:shadow-[0_0_8px_4px_#5B2333] cursor-pointer h-80 w-full"
              onClick={() => {
...
function ServicesItem({ name }: { name: string }) {
  return (
